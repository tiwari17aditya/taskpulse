...
import { Share2, Copy, Check, ExternalLink, X, Shield, ArrowRight } from 'lucide-react';
    ...
    try {
      const res = await fetch('/api/share', {
        ...
        body: JSON.stringify({
          ...
          redirectUrl: shareType === 'redirect' ? redirectUrl : null,
      ...
      <div className="bg-slate-900 border border-slate-800 w-full max-w-lg rounded-2xl p-6 shadow-2xl space-y-5 animate-slide-up relative">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-base font-semibold text-slate-100">Codeshare & Toffeeshare Redirect</h3>
              <p className="text-[11px] text-slate-400">Instant secret link generation & web redirection</p>
            </div>
          </div>
        ...
        {!generatedShare ? (
          <form onSubmit={handleGenerate} className="space-y-4">
            ...
            {shareType === 'code' ? (
              <div className="space-y-1">
                ...
                <textarea
                  ...
                  placeholder="Paste code or note text here..."
                />
              </div>
            ) : (
              <div className="space-y-1">
                <label className="text-xs text-slate-400 font-medium">Target Redirection URL</label>
                <input
                  type="url"
            ...
            >
              {loading ? 'Generating Code...' : 'Generate 6-Digit Share Code'} <ArrowRight className="w-4 h-4" />
            </button>
          </form>
        ) : (
          <div className="space-y-4 text-center py-2">
            <div className="p-4 bg-indigo-950/40 border border-indigo-500/30 rounded-2xl space-y-2">
              <span className="text-[11px] text-indigo-300 font-semibold uppercase tracking-wider">Secret Access Code</span>
              <div className="text-3xl font-mono font-bold text-white tracking-widest">{generatedShare.code}</div>
            </div>

            <div className="space-y-1 text-left">
              <label className="text-xs text-slate-400">Shareable Redirect URL</label>
            ...
            <div className="pt-2 flex justify-between items-center text-xs">
              ...
              >
                Test Redirect Link <ExternalLink className="w-3 h-3" />
              </a>
              <button
