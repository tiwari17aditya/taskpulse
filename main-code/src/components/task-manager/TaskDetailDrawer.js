...
import { CheckCircle2, Circle, X, Sun, Star, Calendar, Check, Tag, Trash2, Repeat, Clock, AlertCircle } from 'lucide-react';
...
export default function TaskDetailDrawer({
  ...
  const handleToggleStarWithDueDate = (e) => {
    // If not starred yet and has no due date, automatically assign today's due date to enforce the priority rule
    if (!selectedTask.starred && !selectedTask.dueDate) {
      updateTaskDueDate?.(selectedTask.id, getTodayStr ? getTodayStr() : new Date().toISOString().split('T')[0]);
    }
    toggleStar?.(selectedTask.id, e);
  };
      ...
      <div className="w-full max-w-md lg:w-80 xl:w-96 bg-slate-900 border border-slate-800 rounded-2xl p-5 shadow-2xl flex flex-col justify-between space-y-4 animate-fade-in shrink-0 h-full max-h-[85vh] lg:max-h-none overflow-y-auto">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              onClick={handleToggleStarWithDueDate}
              className={`py-2 px-3 rounded-xl border text-xs font-medium flex items-center justify-center gap-1.5 transition cursor-pointer ${
            ...
            >
              ...
              {selectedTask.starred ? 'Priority (Starred)' : 'Set Priority ⭐'}
            </button>
          </div>

          {/* Safe Repeat Schedule & Iterations Info (If routine or recurring) */}
            ...
            <div className="p-2.5 bg-indigo-950/30 border border-indigo-500/30 rounded-xl space-y-1 text-xs">
              <div className="flex items-center justify-between text-indigo-300 font-semibold">
                ...
                <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 bg-indigo-500/20 rounded">
                  {selectedTask.frequency || 'Daily'} {Number(selectedTask.interval) > 1 ? `(Every ${selectedTask.interval})` : ''}
                </span>
              </div>
              {selectedTask.targetTime && (
                <div className="flex items-center gap-1 text-[11px] text-slate-400">
                  <Clock className="w-3 h-3 text-amber-400" />
                  <span>Scheduled Time: <span className="font-mono text-slate-200">{selectedTask.targetTime}</span></span>
                </div>
              )}
          ...
          <div className="space-y-2 border-t border-slate-800 pt-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                <Calendar className="w-3.5 h-3.5 text-indigo-400" /> Due Date {selectedTask.starred && <span className="text-amber-400 text-[10px] lowercase font-normal">(required for priority)</span>}
              </span>
              {selectedTask.dueDate && !selectedTask.starred && (
            ...
            <div className="grid grid-cols-3 gap-1.5">
              <button
                type="button"
                onClick={() => updateTaskDueDate?.(selectedTask.id, getTodayStr ? getTodayStr() : new Date().toISOString().split('T')[0])}
                className={`py-1.5 rounded-lg border text-[11px] font-medium transition cursor-pointer ${
                  selectedTask.dueDate === (getTodayStr ? getTodayStr() : '') || selectedTask.dueDate === 'Today'
              ...
              <button
                type="button"
                onClick={() => updateTaskDueDate?.(selectedTask.id, getTomorrowStr ? getTomorrowStr() : '')}
                className={`py-1.5 rounded-lg border text-[11px] font-medium transition cursor-pointer ${
                  selectedTask.dueDate === (getTomorrowStr ? getTomorrowStr() : '') || selectedTask.dueDate === 'Tomorrow'
                    ? 'bg-indigo-600 border-indigo-500 text-white shadow-sm'
                    : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              <button
                type="button"
                onClick={() => updateTaskDueDate?.(selectedTask.id, getNextWeekStr ? getNextWeekStr() : '')}
                className={`py-1.5 rounded-lg border text-[11px] font-medium transition cursor-pointer ${
                  selectedTask.dueDate === (getNextWeekStr ? getNextWeekStr() : '') || selectedTask.dueDate === 'Next Week'
                    ? 'bg-indigo-600 border-indigo-500 text-white shadow-sm'
                    : 'bg-slate-950 border-slate-800 text-slate-400 hover:text-slate-200'
                }`}
              >
            ...
            <div className="flex items-center gap-2 pt-1 bg-slate-950/60 p-2 rounded-xl border border-slate-800/80">
              <span className="text-[11px] text-slate-400 shrink-0">Custom Date:</span>
              <input
                type="date"
                value={selectedTask.dueDate || ''}
                onChange={(e) => updateTaskDueDate?.(selectedTask.id, e.target.value)}
                className="bg-slate-900 border border-slate-700 text-xs text-slate-200 px-2.5 py-1 rounded-lg outline-none focus:border-indigo-500 flex-1 cursor-pointer"
              />
            </div>
          </div>
